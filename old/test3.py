...
big_business_df = pd.read_excel('big_names.xlsx')
...
big_business_list = big_business_df['Название компании'].tolist()

def create_dictionary(first_list, second_list):
    dictionary = {'Название компании': [], 'Top 500': [], 'Сходство (%)': []}
    for word in first_list:
        closest_match, top_500_company, percentage = get_closest_match(word, second_list)
        if closest_match is not None:
            dictionary['Название компании'].append(word)
            dictionary['Top 500'].append(top_500_company)
            dictionary['Сходство (%)'].append(percentage)
    return dictionary
...
def get_closest_match(word, second_list):
    ...
    closest_match = None
    top_500_company = None
    for candidate in second_list:
        similarity = similarity_percentage(word, candidate)
        if similarity > highest_similarity:
            highest_similarity = similarity
            closest_match = candidate
            top_500_company = candidate
    return closest_match, top_500_company, highest_similarity

result_dictionary = create_dictionary(big_business_list, top_500_list)
result_df = pd.DataFrame(result_dictionary)
merged_df = big_business_df.merge(result_df, how='left', left_on='Название компании', right_on='Название компании')
merged_df.to_excel('output_total_1.xlsx', index=False)
